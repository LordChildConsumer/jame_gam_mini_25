...
@onready var grave_material = $GraveSprite.material
...
func appear_from_ground():	
	if grave_material is ShaderMaterial:
		...
		tween.tween_method(update_shader_progress, 0.0, 0.3, 3)  # 1.5s duration

		# Trigger the dirt particle effect at the start of the animation
		dirt_particles.emitting = true
		var dirt_tween = create_tween()
		dirt_tween.tween_property(dirt_particles, "emitting", false, 5)  # Stop emitting after 1.5s
		var dirt_position_tween = create_tween()
		dirt_position_tween.tween_property(dirt_particles, "position", Vector2(dirt_particles.position.x, 60), 5)
	else:
